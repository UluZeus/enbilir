...
const productionEnv = {
  ...
  DATABASE_URL: "mysql://enbilir_app:synthetic@mysql.internal:3306/enbilir_production",
  MYSQL_DEFAULTS_FILE: "/etc/enbilir/mysql-client.cnf",
...
describe("runtime configuration validation", () => {
    ...
    const issues = getRuntimeConfigIssues(
      {
        ...
        MYSQL_DEFAULTS_FILE: "/srv/enbilir/app/mysql.cnf",
  ...
  it("applies the same fail-closed validation to staging deployments", () => {
    const stagingEnv = {
      ...
      DATABASE_URL: "mysql://enbilir_app:synthetic@mysql-staging.internal:3306/enbilir_staging",
      MYSQL_DEFAULTS_FILE: "/etc/enbilir-staging/mysql-client.cnf",
      MYSQL_DATABASE: "enbilir_staging",
