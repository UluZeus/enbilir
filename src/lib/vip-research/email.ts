...
export async function sendVipResearchEmails(reportId: string) {
  ...
  for (const recipient of recipients) {
    const existing = await prisma.vipResearchEmailLog.findUnique({
    ...
    const text = [
      ...
      "",
      ...report.ideas.map((idea) => `${idea.rank}. ${idea.symbol} · ${idea.stance} · Güven ${idea.confidenceScore}/100 · Risk ${idea.riskScore}/100 · Giriş ${formatPrice(idea.entryLow)}-${formatPrice(idea.entryHigh)} · Stop ${formatPrice(idea.stopLoss)} · Hedef ${formatPrice(idea.targetPrice)}`),
      "",
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#172033;max-width:760px;margin:auto">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead><tr style="background:#172033;color:white"><th>Sembol</th><th>Not</th><th>Güven</th><th>Risk</th><th>Giriş</th><th>Stop / Hedef</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
