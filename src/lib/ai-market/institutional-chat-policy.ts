...
function tierInstruction(
  ...
  if (locale === "en") {
    return tier === "VIP"
      ? webResearchRequired ? [
          ...
          "VIP site-help boundary: answer this site, membership, account, or navigation question only from ENBILIR_SITE_CONTEXT and the quoted conversation.",
          "Web research and citations are not required for this operational question. Do not invent account state, payment state, routes, or site capabilities that are absent from the supplied context.",
          "VIP_RESEARCH_CONTEXT is private VIP evidence and may be discussed only because the server has confirmed an active VIP membership.",
        ].join(" ")
      : [
  ...
  return tier === "VIP"
    ? webResearchRequired ? [
        ...
        "VIP site-yardım sınırı: Bu site, üyelik, hesap veya navigasyon sorusunu yalnız ENBILIR_SITE_CONTEXT ve alıntılanan konuşmadan yanıtla.",
        "Bu operasyonel soru için web araştırması ve kaynak bağlantısı zorunlu değildir. Verilen bağlamda bulunmayan hesap durumunu, ödeme durumunu, rotayı veya site özelliğini uydurma.",
        "VIP_RESEARCH_CONTEXT yalnız sunucu aktif VIP üyeliğini doğruladığı için kullanılabilen özel VIP kanıtıdır.",
      ].join(" ")
    : [
...
export function buildInstitutionalChatInstruction(
  locale: MarketChatLocale,
  tier: InstitutionalChatTier,
  webResearchRequired = tier === "VIP",
) {
  const tierRule = tierInstruction(locale, tier, webResearchRequired);
...
export function requiresVipWebResearch(question: string) {
  const normalized = normalizeQuestionForIntent(question);
  const navigationIntent =
    /\b(nasil giderim|nasil ulasirim|nereden acarim|nerede bulurum|hangi menude|hangi sayfada|navigate|navigation|where (?:can|do) i find|how (?:can|do) i (?:open|reach|navigate))\b/.test(normalized);
  const membershipOrAccountIntent =
    /\b(uyeligim|uyelik|aboneligim|abonelik|sorgu hakkim|sorgu kotam|hesabim|hesap ayarlari|giris yap|sifrem|odeme durumum|faturam|membership|subscription|query quota|my account|account settings|sign in|password|billing)\b/.test(normalized);
  const currentMarketIntent =
    /\b(guncel|bugun|simdi|canli|fiyat|piyasa|yatirim|hisse|borsa|kripto|bitcoin|btc|ethereum|eth|altin|gumus|dolar|euro|doviz|faiz|enflasyon|tahvil|emtia|katalizor|bilan[cç]o|al|sat|tut|current|today|live|price|market|invest|stock|equity|crypto|gold|silver|currency|inflation|bond|commodity|catalyst|earnings|buy|sell|hold)\b/.test(normalized);

  if (navigationIntent) {
    return false;
  }

  if (membershipOrAccountIntent && !currentMarketIntent) {
    return false;
  }
...
export function buildInstitutionalOpenAiRequest({
  ...
  const webResearchRequired = tier === "VIP" && requiresVipWebResearch(question);
  ...
  const body: Record<string, unknown> = {
    model,
    instructions: buildInstitutionalChatInstruction(locale, tier, webResearchRequired),
    input: [
      {
        role: "user",
        content: [
