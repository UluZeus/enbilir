...
export function GuidedHelp({ locale, userId, progress }: GuidedHelpProps) {
  ...
  return (
    <>
      {isOpen ? <div aria-hidden="true" onClick={closeHelp} className="fixed inset-0 z-[90] bg-slate-950/60 backdrop-blur-[2px]" /> : null}
    <div className="guided-help-v3 fixed inset-x-3 bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-[100] flex flex-col items-end gap-3 md:inset-x-auto md:bottom-5 md:right-5">
      {isOpen ? (
        <section ref={dialogRef} id={dialogId} role="dialog" aria-modal="true" aria-labelledby={titleId} className="guided-help-dialog-v3 max-h-[calc(100dvh-7rem)] w-full overflow-y-auto md:max-h-[calc(100dvh-2.5rem)] md:w-[390px]">
          <div className="flex items-start justify-between gap-4 border-b border-white/10 bg-[#0b1320] p-4 text-white">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-teal-200">{locale === "tr" ? "Enbilir yardımcısı" : "Enbilir guide"}</p>
              <h2 id={titleId} className="mt-1 text-lg font-bold text-white">{locale === "tr" ? "Sıradaki en doğru adım" : "Your best next step"}</h2>
            </div>
            <button ref={closeButtonRef} type="button" onClick={closeHelp} aria-label={locale === "tr" ? "Yardımı kapat" : "Close help"} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/20 bg-white/10 text-lg font-semibold text-white">×</button>
          </div>
          <div className="p-4">
            {isSignedIn && progress ? (
              <div className="mb-4">
                <div className="flex items-center justify-between text-xs font-semibold text-slate-600"><span>{locale === "tr" ? "Başlangıç ilerlemesi" : "Getting-started progress"}</span><span>{progress.completedCount}/{progress.totalCount}</span></div>
            ...
            {activeStep ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-teal-700">{locale === "tr" ? "Önerilen adım" : "Recommended step"}</p>
                <h3 className="mt-2 text-lg font-bold text-slate-950">{activeStep.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-700">{activeStep.body}</p>
                {activeStep.id === "register" ? (
                  <Link href={activeStep.href} onClick={closeHelp} className="button-primary-v3 mt-4 inline-flex w-full items-center justify-center px-4 py-3 text-sm font-bold">{activeStep.action}</Link>
                ) : (
                  <Link href={activeStep.href} onClick={closeHelp} className="button-primary-v3 mt-4 inline-flex w-full items-center justify-center px-4 py-3 text-sm font-bold">{activeStep.action}</Link>
                )}
                {!isSignedIn ? <Link href={`/${locale}/giris`} onClick={closeHelp} className="mt-2 inline-flex w-full items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800">{locale === "tr" ? "Zaten üyeyim, giriş yap" : "I already have an account"}</Link> : null}
              </div>
            ) : null}
            {isSignedIn ? <Link href={`/${locale}/baslangic`} onClick={closeHelp} className="mt-3 inline-flex text-sm font-bold text-teal-700">{locale === "tr" ? "Tüm başlangıç adımlarını aç" : "Open all getting-started steps"}</Link> : null}
          </div>
        </section>
      ) : null}
      <button type="button" onClick={() => setIsOpen((current) => !current)} aria-expanded={isOpen} aria-controls={dialogId} className="guided-help-trigger-v3 hidden h-11 items-center gap-2 px-3.5 text-sm font-semibold md:inline-flex">
        <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-white/30" aria-hidden="true">?</span>{locale === "tr" ? "Yardım" : "Help"}
      </button>
    </div>
    </>
  );
