  ...
  return (
    <div className="space-y-8">
            ...
            <article key={idea.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm">
              <div className="border-b border-slate-200 bg-white p-6 md:p-8">
                <div className="flex flex-wrap items-start justify-between gap-5">
                  <div>
                    ...
                    <span className={`mt-3 inline-flex rounded-full border px-3 py-1 text-xs font-black ${stanceTone(idea.stance)}`}>{idea.stance}</span>
                  </div>
              ...
              <div className="grid gap-6 p-6 md:grid-cols-2 md:p-8">
                <section className="rounded-2xl border border-slate-200 bg-white p-5">
                  <h3 className="font-black text-slate-950">A) {isEnglish ? "Fundamental mathematics" : "Temel matematik"}</h3>
              ...
              <div className="grid gap-5 border-t border-slate-200 bg-white p-6 md:grid-cols-2 md:p-8">
                <section>
                  <h3 className="font-black text-slate-950">{isEnglish ? "Institutional perception & short data" : "Kurumsal algı ve short verisi"}</h3>
              ...
              <div className="border-t border-slate-200 bg-slate-100 p-6 md:p-8">
                ...
                <p className="mt-1 text-xs text-slate-500">{isEnglish ? "Return is measured from the report's reference price, not a claimed execution price." : "Getiri, gerçekleştiği varsayılan işlemden değil rapor referans fiyatından ölçülür."}</p>
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-100 text-xs uppercase text-slate-500"><tr><th className="px-5 py-3">#</th><th className="px-5 py-3">Varlık</th><th className="px-5 py-3">Not</th><th className="px-5 py-3">Güven</th><th className="px-5 py-3">Risk</th><th className="px-5 py-3">Giriş</th><th className="px-5 py-3">Stop</th><th className="px-5 py-3">Hedef</th></tr></thead><tbody>{report.ideas.map((idea) => <tr key={idea.id} className="border-t border-slate-100"><td className="px-5 py-4 font-black">{idea.rank}</td><td className="px-5 py-4 font-black">{idea.symbol}</td><td className="px-5 py-4">{idea.stance}</td><td className="px-5 py-4">{idea.confidenceScore}/100</td><td className="px-5 py-4">{idea.riskScore}/100</td><td className="px-5 py-4">{number(idea.entryLow)}-{number(idea.entryHigh)}</td><td className="px-5 py-4 text-rose-700">{number(idea.stopLoss)}</td><td className="px-5 py-4 text-emerald-700">{number(idea.targetPrice)}</td></tr>)}</tbody></table></div>
      </section>
