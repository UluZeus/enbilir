...
function ReportPreview({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/5 p-5" aria-label={isEnglish ? "VIP report preview" : "VIP rapor önizlemesi"}>
      ...
      <div className="absolute inset-x-5 bottom-5 flex items-center justify-center rounded-xl border border-[#e7c977]/30 bg-[#07111f]/92 px-4 py-3 text-center text-xs font-semibold text-[#f3dda0] backdrop-blur">● {isEnglish ? "Full access is free to signed-in members during the launch promotion" : "Tanıtım döneminde giriş yapan üyelere tam erişim ücretsiz"}</div>
    </section>
  );
}
