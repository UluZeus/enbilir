  ...
  return (
    <section className="premium-card p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-black text-[#152033]">Teknik göstergeler</h2>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {rows.map((row) => (
          <div key={row.label} className="rounded-md bg-[#f8fafc] p-3">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">{row.label}</p>
            <p className="mt-1 text-base font-black text-[#152033]">{row.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-2">
        {risk.reasons.map((reason) => (
          <p key={reason} className="rounded-md border border-slate-200 bg-white/70 px-3 py-2 text-sm text-slate-600">
            {reason}
          </p>
        ))}
      </div>
