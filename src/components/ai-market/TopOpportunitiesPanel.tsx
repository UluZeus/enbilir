...
export function TopOpportunitiesPanel({ analyses, isLoading = false, updatedAt }: TopOpportunitiesPanelProps) {
  ...
  return (
    <section className="premium-card overflow-hidden border border-slate-200 bg-[#f8fafc]">
      <div className="flex flex-col gap-2 border-b border-slate-200 bg-white/75 p-4 md:flex-row md:items-end md:justify-between md:p-5">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">AI Piyasa Asistanı</p>
          <h2 className="mt-1 text-lg font-black text-[#152033]">En İyi 5 Fırsat</h2>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-xs font-black text-slate-600">
          <span className="rounded-md border border-slate-200 bg-white px-3 py-2">80+ güçlü sinyal</span>
          <span className="rounded-md border border-slate-200 bg-white px-3 py-2">60-80 takip</span>
          {updatedAt ? <span className="rounded-md border border-slate-200 bg-white px-3 py-2">Güncelleme {formatTime(updatedAt)}</span> : null}
        </div>
      </div>

      {isLoading && opportunities.length === 0 ? <LoadingState /> : null}
      {!isLoading && opportunities.length === 0 ? <EmptyState /> : null}
      {opportunities.length > 0 ? <OpportunityCards opportunities={opportunities} /> : null}
    </section>
  );
}
    ...
    <article className="rounded-md border border-slate-200 bg-white p-3 shadow-sm">
      <div className="grid gap-3 lg:grid-cols-[minmax(150px,1.1fr)_minmax(150px,1fr)_minmax(240px,1.4fr)] lg:items-center">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-2">
          <Metric label="Tavsiye" value={`${opportunity.recommendationScore}/100`} className={getScoreClass(opportunity.recommendationScore)} />
          <Metric label="Risk" value={`${opportunity.riskScore}/100`} className={getRiskClass(opportunity.riskScore)} />
          <Metric label="Aralık" value={opportunity.interval} />
          <Metric label="Fiyat" value={formatPrice(opportunity.lastPrice)} />
        </div>
