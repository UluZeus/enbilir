...
const copy = {
  tr: {
    ...
    standardTier: "Free",
    vipTier: "VIP",
    standardTitle: "Free AI sohbet",
    standardBody: "Tanıtım döneminde tam VIP içerik, site içi kanıt ve güncel kaynaklı internet araştırması tüm giriş yapan üyelere açıktır.",
    vipTitle: "VIP destekçi AI sohbet",
    vipBody: "100 TL aylık gönüllü katkınız AI işletim maliyetlerini destekler; tam VIP içerik kapsamınız Free planda da aynen sürer.",
    vipPay: "100 TL ile destek ol",
    promoBody: "Faturalandırma planınız Free’dir; tanıtım döneminde tam VIP içerik erişiminiz devam eder.",
    freeQuota: "Free planınızda günlük 10 AI sorgu hakkınız var. Haklarınız her gece İstanbul saatiyle 00.00'da sıfırlanır.",
    paidQuota: "100 TL VIP ödemeniz aktif. Günlük toplam 15 AI sorgu hakkınız var; haklarınız her gece İstanbul saatiyle 00.00'da sıfırlanır.",
    remaining: "Bugün kalan",
    upgradeQueries: "100 TL öde, günlük 15 sorguya geç",
    paymentUnavailable: "Güvenli ödeme şu anda kullanılamıyor; Free erişiminiz ve günlük 10 sorgu hakkınız etkilenmez.",
  ...
  en: {
    ...
    standardTier: "Free",
    vipTier: "VIP",
    standardTitle: "Free AI chat",
    standardBody: "During the launch promotion, full VIP content, site evidence, and current cited web research are open to every signed-in member.",
    vipTitle: "VIP supporter AI chat",
    vipBody: "Your voluntary 100 TL monthly contribution supports AI operating costs; your full VIP content scope also remains available on Free.",
    vipPay: "Support with 100 TL",
    promoBody: "Your billing plan is Free; full VIP content access continues during the launch promotion.",
    freeQuota: "Your Free plan includes 10 AI queries per day. Your allowance resets every night at 00:00 Istanbul time.",
    paidQuota: "Your 100 TL VIP payment is active. You have 15 AI queries per day, resetting every night at 00:00 Istanbul time.",
    remaining: "Remaining today",
    upgradeQueries: "Pay 100 TL for 15 daily queries",
    paymentUnavailable: "Secure payment is temporarily unavailable; your Free access and 10 daily queries are unaffected.",
...
export function AiMarketChatPanel({
  ...
  const [paidVipActive, setPaidVipActive] = useState(isPaidVipActive);
  const [quota, setQuota] = useState<AiQueryQuota | null>(null);
  const [upgradeUrl, setUpgradeUrl] = useState<string | null>(isPaidVipActive ? null : `/${safeLocale}/vip?upgrade=queries#ai-query-upgrade`);
  ...
  useEffect(() => {
    ...
    void fetch(`/api/ai-market/chat?locale=${safeLocale}`, {
  ...
  return (
    <section className={`ai-market-chat-panel mx-auto mb-4 w-full max-w-[1600px] rounded-2xl border p-3 text-white shadow-2xl md:p-4 ${paidVipActive ? "border-amber-300/25 bg-[linear-gradient(145deg,#08111f,#111827_68%,#241d10)]" : "border-slate-800 bg-[#08111f]"}`}>
      <header className="flex flex-col gap-2 px-1 pb-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className={`text-[11px] font-bold uppercase tracking-[0.16em] ${paidVipActive ? "text-amber-200" : "text-cyan-200"}`}>{text.kicker}</p>
          <h2 className="mt-1 text-xl font-bold tracking-tight text-white md:text-2xl">{text.title}</h2>
        </div>
        <span className={`w-fit rounded-full border px-3 py-1 text-[11px] font-bold uppercase tracking-[0.12em] ${paidVipActive ? "border-amber-300/25 bg-amber-300/10 text-amber-100" : "border-cyan-300/20 bg-cyan-300/10 text-cyan-100"}`}>
          {paidVipActive ? text.vipTier : text.standardTier} · {mode === null ? text.readyMode : mode === "openai" ? text.aiMode : text.localMode}
        </span>
      </header>
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_260px]">
        <div className="relative flex h-[clamp(540px,70vh,780px)] min-w-0 flex-col overflow-hidden rounded-xl border border-slate-800 bg-slate-950/65">
          ...
          <div className="sticky bottom-0 border-t border-slate-800 bg-[#0a1220]/95 p-2.5 backdrop-blur-xl md:p-3">
            <form onSubmit={handleSubmit} className="grid gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className={`min-h-10 flex-1 rounded-lg px-5 text-sm font-bold text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-55 sm:flex-none ${paidVipActive ? "bg-amber-200" : "bg-cyan-100"}`}
                >
                  {isSending ? "…" : text.send}
                </button>
              </div>
            ...
            <div className="mt-2 min-h-5" aria-live="polite">
              ...
              {quota?.remaining === 0 && !paidVipActive && upgradeUrl ? (
                <a href={upgradeUrl} className="mt-1 inline-flex rounded-lg border border-amber-200/35 bg-amber-200/15 px-3 py-2 text-xs font-bold text-amber-100">
                  {text.upgradeQueries} →
                </a>
              ) : null}
        ...
        <aside className="grid content-start gap-3">
          <section className={`rounded-xl border p-3 ${paidVipActive ? "border-amber-300/25 bg-amber-300/10" : "border-cyan-300/20 bg-cyan-300/8"}`}>
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-white">{paidVipActive ? text.vipTitle : text.standardTitle}</p>
            <p className="mt-2 text-xs font-medium leading-5 text-slate-300">{paidVipActive ? text.vipBody : text.standardBody}</p>
            <p className="mt-2 rounded-lg border border-emerald-300/20 bg-emerald-300/10 px-3 py-2 text-xs font-semibold leading-5 text-emerald-100">{text.promoBody}</p>
            <div className="mt-2 rounded-lg border border-white/10 bg-slate-950/45 px-3 py-2.5">
              <p className="text-xs font-semibold leading-5 text-slate-200">{paidVipActive ? text.paidQuota : text.freeQuota}</p>
              <p className="mt-1 text-sm font-bold text-white">
                {text.remaining}: {quota?.remaining ?? (paidVipActive ? 15 : 10)}/{quota?.limit ?? (paidVipActive ? 15 : 10)}
              </p>
              {!paidVipActive && upgradeUrl ? (
                <a href={upgradeUrl} className="mt-2 inline-flex min-h-9 items-center rounded-lg border border-amber-200/35 bg-amber-200/15 px-3 py-2 text-xs font-bold text-amber-100 transition hover:bg-amber-200/25">
                  {text.upgradeQueries} →
                </a>
              ) : null}
            </div>
            {paidVipActive && vipPaidUntil ? (
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {!paidVipActive && paymentUrl ? (
                <a href={paymentUrl} target="_blank" rel="noreferrer" className="min-h-11 rounded-lg border border-amber-200/30 bg-amber-200/10 px-3 py-2 text-xs font-bold text-amber-100 hover:bg-amber-200/15">{text.vipPay}</a>
              ) : !paidVipActive ? <p className="text-xs font-semibold leading-5 text-slate-300">{text.paymentUnavailable}</p> : null}
            </div>
          </section>
