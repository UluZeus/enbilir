...
function formatCountdown(milliseconds: number, locale: "tr" | "en") {
  ...
  return locale === "en"
    ...
    : `${String(hours).padStart(2, "0")} saat ${String(minutes).padStart(2, "0")} dakika ${String(seconds).padStart(2, "0")} saniye`;
}

export function MacroReportTicker({ variant = "light", locale = "tr" }: { variant?: "light" | "dark"; locale?: "tr" | "en" }) {
  const [now, setNow] = useState<Date | null>(null);
  const message = now
    ? (() => {
        const nextReportTime = getNextReportTime(now);
        const countdown = formatCountdown(nextReportTime.getTime() - now.getTime(), locale);
        const nextReportLabel = new Intl.DateTimeFormat(locale === "en" ? "en-US" : "tr-TR", {
          hour: "2-digit",
          minute: "2-digit",
    ...
    : locale === "en"
      ? "The schedule for Dr. Hakan Ünsal's next macro report is being calculated. The comments and views here are not investment advice; market data may be delayed, incomplete, or inaccurate."
  ...
  return (
    <section className={`macro-report-ticker overflow-hidden rounded-md border shadow-lg backdrop-blur ${tone}`} aria-label={locale === "en" ? "Macro report countdown" : "Makro rapor geri sayımı"}>
