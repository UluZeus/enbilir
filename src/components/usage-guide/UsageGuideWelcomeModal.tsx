...
export function UsageGuideWelcomeModal({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto bg-slate-950/62 px-4 pb-6 pt-[calc(env(safe-area-inset-top)+1rem)] backdrop-blur-sm md:py-8">
      <section className="w-full max-w-5xl overflow-hidden rounded-2xl border border-white/70 bg-white shadow-2xl">
        <div className="bg-[#08111f] p-5 text-white md:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#67e8f9]">{content.eyebrow}</p>
              <h2 className="mt-2 text-2xl font-black md:text-4xl">{content.modalTitle}</h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-200">{content.modalBody}</p>
            </div>
            <button
              type="button"
              onClick={close}
              aria-label={locale === "tr" ? "Kılavuzu kapat" : "Close guide"}
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/20 bg-white/10 text-xl font-black text-white hover:bg-white/20"
            >
              x
            </button>
          </div>
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            ...
            <button type="button" onClick={close} className="rounded-xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-black text-white">
              {content.closeLabel}
            </button>
          </div>
        </div>
