...
import { VipQueryUpgradeCard } from "@/components/vip/VipQueryUpgradeCard";
...
export default async function VipPage({
  ...
  if (!canView) {
    ...
    return <div className="grid gap-5"><FormMessage message={query.error ?? query.message} tone={query.message ? "success" : "error"} /><VipPaywall locale={locale} isSignedIn={Boolean(session)} latestClaim={latestClaim} paymentUrl={paymentUrl} /></div>;
  }

  const latestClaim = membership?.isPaidVipActive ? null : await prisma.vipSubscriptionClaim.findFirst({
    where: { userId: session!.id },
    orderBy: { createdAt: "desc" },
    select: { status: true, providerReference: true, createdAt: true },
  });
  ...
  return (
    <div className="space-y-9">
      ...
      <VipResearchReportView report={serializeVipResearchReport(latest)} locale={locale} />
